...
export function SocietyCard({ society }: SocietyCardProps) {
  ...
  return (
    <div className="group relative flex flex-col rounded-3xl bg-slate-900/80 border border-slate-800/80 overflow-hidden shadow-xl hover:border-slate-700 hover:shadow-2xl hover:shadow-blue-500/10 transition-all duration-300 hover:-translate-y-1">
      {/* Card Banner Image */}
      <div className="relative h-36 w-full overflow-hidden bg-slate-800">
        ...
        <div className="absolute top-3 left-3 right-3 flex items-center justify-between">
          <span className={`text-[10px] uppercase font-bold tracking-wider px-2.5 py-1 rounded-full border backdrop-blur-md ${catStyle.bg} ${catStyle.text} ${catStyle.border}`}>
            {society.category.replace("_", " ")}
          </span>

          {isOpen ? (
            <span className="inline-flex items-center space-x-1 text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 backdrop-blur-md">
        ...
        <div className="absolute -bottom-5 left-6">
          <div className="w-14 h-14 rounded-2xl bg-slate-900 p-1 border-2 border-slate-700 shadow-xl overflow-hidden">
            <img
      ...
      <div className="flex-1 flex flex-col p-6 pt-8 space-y-3">
        ...
        <div className="pt-2 mt-auto">
          <Link
            ...
            className="w-full flex items-center justify-center space-x-1.5 py-2.5 px-4 rounded-xl text-xs font-bold text-white bg-slate-800/90 hover:bg-blue-600 hover:shadow-lg hover:shadow-blue-500/20 active:scale-98 transition-all border border-slate-700/80 hover:border-blue-500/40 group-hover:bg-blue-600"
          >
